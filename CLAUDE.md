...
- **Agenda** : calendriers iCloud publiés (URL ICS `webcal://` → `https://`), récurrences gérées par node-ical (`rrule.between`), fenêtre de 7 jours coupée en deux sections : « aujourd'hui » (2 max, heure seule) et « à venir » (2 max, libellés relatifs "demain 18 h", "lun 27 · 19 h 30"). Multi-calendriers via `config.agendas` [{url, qui}] (repli sur `icsUrl` seul) : maison + persos fusionnés chronologiquement partout, chargés en `Promise.allSettled` (un calendrier mort ne bloque pas les autres, échec global seulement si tous échouent). Les événements perso sont signés — suffixe `· prénom` dans l'arche (plafond 2+2 inchangé), pastille couleur ménage (olive Antoine / terracotta Inès, initiale du `qui`) dans la vue semaine ; `maison` reste nu
