...
- Navigation multi-vues côté client (`.vue`, fonction `montrer(id)`) : chaque écran de détail a un « ‹ accueil », et retour automatique à l'écran principal après 5 min sans interaction — sauf minuteur en marche
- **Minuteur de cuisine** : double-toucher l'horloge → plein écran (chiffres Optima géants), presets 1/5/10/15/20/30 min qui lancent aussitôt, +1 min, pause/reprise, annuler. Fin = fond qui pulse (pas de son, autoplay iOS peu fiable) ; si on est sur un autre écran quand ça sonne, le minuteur se raffiche tout seul. « ‹ accueil » le laisse tourner en arrière-plan
- **Moments de l'année** : petite ligne Georgia italique sous la date + **glissement de palette** de tout l'écran (intégrés : 20-23/12 « c'est bientôt noël », 24-26/12 « joyeux noël », 31/12, 01/01, 14/02 ; personnalisés via `config.moments` [{jour:"MM-DD", texte}], **prioritaires** sur les fêtes du calendrier), calculés côté serveur (`momentDuJour`), envoyés via `/contenu` (`moment` : {cls, texte}), classe `m-*` posée sur body. Thèmes (jour et nuit) : `m-noel` olive → sapin, terracotta → rouge profond ; `m-coeur` terracotta → framboise, ivoire rosé ; `m-fete` sable → doré. Petit motif dessiné en CSS pur à côté du vœu (sapin / cœur / bougie), aucun emoji. Ornements suspendus (`.deco-fete`, 5 fils tombant du bord haut dans les respirations qui encadrent le pendentif, léger balancement en `animation`) : boules de noël, cœurs, sequins losanges selon le thème — masqués hors jours de fête
- **Watchdog** : l'ipad polle toutes les 2 s → `dernierPoll` horodaté sur `/musique/etat` et `/contenu` ; si silence > 30 min, alerte unique via `config.alerteUrl` (webhook texte type ntfy.sh, POST du corps), puis « de retour » à la reprise. Vérif toutes les 5 min
- **Anti-marquage LCD** : tout le `#stage` glisse d'1 px selon un cycle de 4 positions toutes les 4 min (translate intégré au transform de `fit()`), imperceptible
...
- **Météo** : Open-Meteo, gratuit sans clé (7 jours de prévision), codes météo mappés en libellés FR. Ligne détail : mini/maxi du jour. La zone météo de l'accueil est tactile → menu « météo & maison » (deux cartes en arches : « les jours à venir » olive, « chauffage » terracotta tamisée marquée bientôt — réservée au futur pilotage chaudière/thermostats, écran placeholder). Écran prévisions : 7 colonnes pré-rendues côté serveur (`donnees.meteo.previsions`, envoyées via `/contenu` `meteoPrevisions`) — jour, icône météo dessinée en CSS pur (classes `ico-*` mappées par `iconeMeteo(code)` : soleil/éclaircies/couvert/brouillard/pluie/neige/orage), libellé, maxi/mini, proba de pluie si ≥ 30 %, vent (terracotta si ≥ 40 km/h) et UV si ≥ 6 ; ligne lever/coucher du soleil sous le titre (composée côté client depuis `soleil`). Ligne conseil (Georgia italique, sous la météo) : phrase amicale et tutoiement, format « info · geste », l'heure toujours présente (« averses vers 17 h · prends le parapluie »). Fonction pure `conseilMeteo(v)`, déterministe (pas d'aléatoire : la zone clignoterait au poll). Selon l'heure de Paris — matin → la journée (parapluie/chaleur ≥ 30°/petite laine ≤ 10°), après-midi → la soirée (parapluie, pull si ≤ 16° ou grosse chute et ≤ 18°), soir → fin de soirée puis demain (parapluie au départ, volets si ≥ 30°, couvre-toi si chute ≥ 6°). Seuil pluie proba ≥ 50 %
