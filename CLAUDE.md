...
- **Moments de l'année** : petite ligne Georgia italique terracotta sous la date (intégrés : 20-23/12 « c'est bientôt noël », 24-26/12 « joyeux noël », 31/12, 01/01, 14/02 ; personnalisés via `config.moments` [{jour:"MM-DD", texte}]), calculée côté serveur (`momentDuJour`), envoyée via `/contenu` (`moment` : {cls, texte}), classe `m-*` posée sur body
