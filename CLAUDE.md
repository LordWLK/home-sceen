...
- **Moments de l'année** : petite ligne Georgia italique sous la date + **glissement de palette** de tout l'écran (intégrés : 20-23/12 « c'est bientôt noël », 24-26/12 « joyeux noël », 31/12, 01/01, 14/02 ; personnalisés via `config.moments` [{jour:"MM-DD", texte}], **prioritaires** sur les fêtes du calendrier), calculés côté serveur (`momentDuJour`), envoyés via `/contenu` (`moment` : {cls, texte}), classe `m-*` posée sur body. Thèmes (jour et nuit) : `m-noel` olive → sapin, terracotta → rouge profond ; `m-coeur` terracotta → framboise, ivoire rosé ; `m-fete` sable → doré. Petit motif dessiné en CSS pur à côté du vœu (sapin / cœur / bougie), aucun emoji
