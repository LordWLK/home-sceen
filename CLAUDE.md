...
- **Météo** : Open-Meteo, gratuit sans clé (2 jours de prévision), codes météo mappés en libellés FR. Ligne détail : mini/maxi du jour. Ligne conseil (Georgia italique, sous la météo) : phrase sobre et adaptative selon l'heure de Paris — matin → l'après-midi (temp max + pluie/chaleur/fraîcheur), après-midi → la soirée (temp à 21 h + averses), soir/nuit → demain (mini/maxi + libellé ou pluie). Seuil pluie proba ≥ 50 %
...
- **Ciné** : pages horaires AlloCiné des cinémas de `cinemas` (codes `salle_gen_csalle`), extraction regex des notes presse/spectateurs, avec les salles où le film passe (noms joints par « + », « N salles » au-delà de 2). Mix (arche) : 2 meilleures nouveautés + 2 meilleures reprises (détection par mention « reprise » ou année ≤ N-2 dans l'entête de carte, année affichée pour les classiques), complété si une catégorie manque. Écran de détail : une page horaires par jour et par cinéma (aujourd'hui = `.html`, jours suivants = `/d-N/`), séances extraites des attributs `data-showtime-time` (ISO déjà en heure de Paris) et filtrées sur la date attendue (protège si allociné sert la page du jour pour une url inconnue). Pré-rendu côté serveur en 3 variantes (aujourd'hui/demain/surlendemain) × une colonne par cinéma, triées par note ; le sélecteur de jour bascule côté client sans refetch. Colonnes à défilement interne (les longues listes ne débordent pas de l'écran). Rafraîchi toutes les 6 h. Scraping fragile par nature : en cas de changement de structure allociné, `[maj] ciné en échec` et l'arche garde son dernier contenu
